...
const OTPConfirmation = () => {
    ...
    const id = searchParams.get("identifer");
    const { t } = useLanguage();
    ...
    const confirmOtp = useMutation({
        mutationFn: (otp) => authService.verfiyPasswordOtp(otp, id),
        onSuccess: (data) => {
            navigate(`/auth/reset-password?resetToken=${encodeURIComponent(data.resetToken)}&identifer=${id}`, { replace: true });
        },
        onError: (error) => {
    ...
    const onSubmit = async (data) => {
        ...
        await confirmOtp.mutateAsync(data.otp, id);
